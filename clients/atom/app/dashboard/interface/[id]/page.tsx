...
export default function InterfacePage({ params }: { params: { id: string } }) {
  const [interfaceList, setInterfaceList] = useState([]);
  const [menuList, setMenuList] = useState<any>([]);
  const [site, setSite] = useState<ISite>({} as ISite);
  ...
  const onClick = (e: { key: string }) => {
    const current = interfaceList.filter((inter: IInterface) => inter.id === e.key)[0];
